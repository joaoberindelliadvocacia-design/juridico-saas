...
export default function ModalPrazo({ onClose, onSalvo, processoPreSelecionado, dadosDjen }: ModalPrazoProps) {
  ...
  const labelStyle = { fontSize: '13px', fontWeight: 600 as const, color: '#374151', display: 'block' as const, marginBottom: '6px' }
  const toggleStyle = (ativo: boolean) => ({
    width: '40px', height: '22px', borderRadius: '11px', border: 'none', cursor: 'pointer',
    backgroundColor: ativo ? '#8B7536' : '#cbd5e1', position: 'relative' as const, flexShrink: 0,
        ...
        {sucesso ? (
          <div style={{ padding: '48px 24px', textAlign: 'center' }}>
            ...
            <p style={{ color: '#64748b', fontSize: '14px', marginTop: '8px' }}>
              {alertaDias5 || alertaDias7 ? 'Alertas por e-mail agendados.' : ''}
            </p>
          </div>
        ) : (
          <form onSubmit={handleSubmit} style={{ padding: '20px 24px' }}>
              ...
              {processoSelecionado ? (
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '10px 12px', backgroundColor: '#f8fafc', borderRadius: '8px', border: '1px solid #e2e8f0' }}>
                  ...
                  {!processoPreSelecionado && (
                    <button type="button" onClick={() => { setProcessoSelecionado(null); setBuscaProcesso('') }} style={{ fontSize: '12px', color: '#8B7536', background: 'none', border: 'none', cursor: 'pointer', fontWeight: 600 }}>Alterar</button>
                  )}
                </div>
              ) : (
            ...
            <div style={{ display: 'flex', gap: '10px', justifyContent: 'flex-end' }}>
              <button type="button" onClick={onClose} style={{ padding: '10px 20px', borderRadius: '8px', fontSize: '14px', fontWeight: 600, border: '1px solid #e2e8f0', backgroundColor: 'white', color: '#64748b', cursor: 'pointer' }}>
                Cancelar
              </button>
